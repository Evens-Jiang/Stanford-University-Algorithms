...
adjlist_node_p createNode(int v, int d)
{
...
void addEdge(graph_t *graph, int src, int dest, int d)
{
    /* Add an edge from src to dst in the adjacency list*/
    adjlist_node_p newNode = createNode(dest, d);
...
void displayGraph(graph_p graph)
{
    ...
    for (i = 1; i <= graph->num_vertices; i++)
    {
        ...
        while (adjListPtr)
        {
            printf("%d(distance = %d)->", adjListPtr->vertex, adjListPtr->distance);
            adjListPtr = adjListPtr->next;
        }
        printf("NULL\n");
    }
