...
adjlist_node_p createNode(int v, int cost) {
...
void addEdge(graph_t *graph, int src, int dest, int cost)
{
    /* Add an edge from src to dst in the adjacency list*/
    adjlist_node_p newNode = createNode(dest, cost);
...
void displayGraph(graph_p graph)
{
    ...
    for (i = 1; i <= graph->num_vertices; i++)
    {
        ...
        while (adjListPtr)
        {
            printf("%d (cost = %d)->", adjListPtr->vertex, adjListPtr->cost);
            adjListPtr = adjListPtr->next;
        }
        printf("NULL\n");
    }
